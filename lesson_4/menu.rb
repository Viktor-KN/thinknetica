class Menu
  attr_reader :name, :menu_list, :data_table

  def initialize(name, menu_list, data_table)
    @name = name
    @menu_list = menu_list
  ...
  def run
    ...
    loop do
      print_nn name
      choice = ask(your_choice, menu_list[current_menu])

      if menu_list[current_menu][choice][:action] == :show_menu
        current_menu = menu_list[current_menu][choice][:params][0]
        next
      else
        action = menu_list[current_menu][choice][:action]
        params = []
        menu_list[current_menu][choice][:params].each do |param|
  ...
  def incorrect_value
    'Incorrect value provided. Try again: '
  end
