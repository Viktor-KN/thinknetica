...
class Menu
  ...
  validate :menu_list, :type, Hash
  validate :data_table, :type, Hash
  ...
  def validate_menu_list
    menu_list.each do |key, value|
      ...
      raise "No mandatory :title key for menu_list[:#{key}]." unless value.key?(:title)
      raise "No mandatory :menu key for menu_list[:#{key}]." unless value.key?(:menu)
      raise "menu_list[:#{key}][:menu] must be a Hash." unless menu_list[key][:menu].is_a?(Hash)
      raise "menu_list[:#{key}][:menu] must not be empty" if menu_list[key][:menu].empty?
  ...
  def validate_menu(title, menu)
    menu.each do |key, value|
      raise "Item with '#{key}' in #{title} must be a Hash." unless value.is_a?(Hash)
